...
- [ ] **3.1 — Decomposed model v2** — SPEC §11.4. Only after v1 is fully evaluated. Compare against v1 on the same harness; keep whichever wins. **Stage 1 (team environment) and Stage 2 (opportunity) of the 4-stage pipeline are both built and evaluated for real against 2021-2025 data — not checked off, since the decision on whether to continue to Stage 3 is still open.** Stage 1 beats its real bar (`trailing_ewm_4`) on both `team_plays`/`pass_rate`, but loses to a plain `league_mean` on both. Stage 2 loses to its real bar (`trailing_raw`) on all three outputs (`targets`/`carries`/`rz_touches`), beats `league_mean` on `targets`/`carries`, loses to it on `rz_touches` (a likely real units-mismatch in that one formula, diagnosed but deliberately left unfixed — see `docs/JOURNAL.md`). Full real numbers and CIs for both stages are in `docs/JOURNAL.md`'s corrected entries (each stage went through a final whole-branch review that caught and fixed a real bug before the numbers above were trusted). See `HANDOFF.md` §3 for the exact resume options.
