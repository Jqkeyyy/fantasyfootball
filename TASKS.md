...
- [ ] **3.1 — Decomposed model v2** — SPEC §11.4. Only after v1 is fully evaluated. Compare against v1 on the same harness; keep whichever wins. **Stage 1 (team environment) and Stage 2 (opportunity) of the 4-stage pipeline are both built and evaluated for real against 2021-2025 data, with every diagnosed bug now fixed (2026-08-15: the `rz_touches` units-mismatch fix closed out the last open one) — not checked off, since the decision on whether to continue to Stage 3 is still open, now with a recommendation attached.** Stage 1 beats its real bar (`trailing_ewm_4`) on both `team_plays`/`pass_rate`, but loses to a plain `league_mean` on both. Stage 2 loses to its real bar (`trailing_raw`) on all three outputs (`targets`/`carries`/`rz_touches`) and beats `league_mean` only on `targets`/`carries` (statistically tied with it on `rz_touches` after the fix, not beating it). **Recommendation: do not proceed to Stage 3 on this foundation** — CLAUDE.md rule 6 ("beat the baselines before believing the model") isn't cleared at either stage, and Stage 3/4 are substantial builds that would compound rather than fix the gap. Full real numbers and CIs for both stages are in `docs/JOURNAL.md`'s corrected entries. See `HANDOFF.md` §3 for the exact resume options — your call, not decided here.
