        ...
        [
            {
                ...
                "team": "KC", "opponent_team": "DEN",
                "mean": mean, "q10": mean - 6, "q25": mean - 3, "q50": mean,
                "q75": mean + 3, "q90": mean + 6, "is_current_week": is_current,
            }
        ]
    )
...
def test_aggregate_ros_p_active_now_discounts_future_weeks_not_current_week() -> None:
    ...
    p_active_now = {"p1": 0.5}
    common_kwargs = dict(
        p_active_now=p_active_now, p_miss_now={"p1": 0.0}, position_by_player={"p1": "RB"},
        calibration=RosCalibration(
            within_player_week_correlation={"RB": 0.3}, recovery_prob={"RB": 0.5}
        ),
        playoff_weeks=[], ros_sims=5000, default_recovery_prob=0.5,
    ...
    current_only = ros_aggregate.aggregate_ros(
        ...
        rng=np.random.default_rng(5), **common_kwargs,
    )
    ...
    future_only = ros_aggregate.aggregate_ros(
        ...
        rng=np.random.default_rng(6), **common_kwargs,
    )
    ...
    combined = ros_aggregate.aggregate_ros(
        _projections_ros(), rng=np.random.default_rng(7), **common_kwargs,
    )
