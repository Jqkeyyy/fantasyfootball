...
**Last machine:** `Maybe` (Windows). Sleeper and every live rankings source reachable here this session (ESPN/FFToday excepted — see below, a pre-existing known gap, not new).
...
**Next task: fixing/replacing broken rankings sources (ESPN, FFToday), then researching up to 5 more sources, even partial-coverage ones (top 50-100 is fine).** Not a numbered TASKS.md task — direct instruction from you this session, prompted by noticing the board looks off with only 3 of 5 sources live. This is also literally ADDENDUM-03 §E's own T-7-days runbook item ("Fix or replace the two dead ranking sources"). In progress as of this handoff — see §3.
...
- [x] Ranking sources chosen — FantasyPros, ESPN, FantasySharks, CBS Sports, FFToday. **FFToday now 403s live; ESPN's bulk endpoint now returns 0 rows.** Board runs on 3 of 5 sources today — actively being worked on, see §3 and "Next task" above.
...
**Rankings-source repair + expansion — started this session, not finished.** You asked, after noticing the board looked off with 2 of 5 sources dead: (1) fix ESPN (0 rows) and FFToday (403) if possible, (2) research up to 5 more ranking sources, partial top-50/100 coverage acceptable. Nothing coded yet as of this handoff line — resume by re-reading `ingest/rankings.py`'s existing `fetch_espn`/`fetch_fftoday` to see their current real failure mode live, then decide fix-vs-replace per source. For new sources, `docs/JOURNAL.md`'s 0.7 entry already documents which sites were tried and rejected (NFL.com, Yahoo, Sharp Football Analysis — none had an accessible plain-HTTP data path) — don't re-investigate those three, look elsewhere.
