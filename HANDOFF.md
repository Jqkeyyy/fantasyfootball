...
**Next task: undecided — the model v2 Stage 1 verification you asked for is done, including a final whole-branch review that found and fixed a real bug in it; the corrected result is still a real, honest mixed outcome, your call on next steps.** Model v2 Stage 1 (team environment, TASKS.md 3.1, SPEC §11.4) is built and verified against real 2021-2025 data. **A final cross-task review after all six of the plan's tasks landed caught a critical bug the per-task reviews had missed: both of `team_plays`'s monotonic constraints (`neutral_pace_ewm_8`, `opponent_neutral_pace_ewm_8`) were registered as *increasing* when the real, verified correlation with `plays` is negative for both (`neutral_pace_sec` is seconds-per-play — higher means slower, i.e. fewer plays) — same failure mode `models.points`'s own docstring already documents guarding against. Also fixed in the same pass: an internal lag-shift inconsistency (trailing features used week-arithmetic shift while this table's own baselines used positional shift, nulling trailing features on bye-week returns baselines stayed populated on) and a missing `season_type == "REG"` scope in the evaluation script (real postseason team-weeks were leaking into training/validation).** Corrected real result (`docs/JOURNAL.md`'s new correction entry has the full account): `team_plays` — `team_env_model` mae=6.9822 (n=2718) vs `trailing_ewm_4` (the real bar) mae=7.4442 (n=2558) vs `league_mean` (sanity floor) mae=6.7236 (n=2718). `pass_rate` — `team_env_model` mae=0.0854 (n=2718) vs `trailing_ewm_4` mae=0.0887 (n=2558) vs `league_mean` mae=0.0850 (n=2718). **Same qualitative conclusion as before the fix: Stage 1 still beats the design's own stated bar (`trailing_ewm_4`) on MAE for both targets — a real pass — but still does not beat the plain `league_mean` sanity floor on either target**, though both gaps narrowed slightly and the model's own MAE improved slightly on both targets now that its constraints point the right direction. Not tuned to force a cleaner number, either before or after the fix. Full account: `docs/JOURNAL.md`'s new entry and its correction sub-bullets, `.superpowers/sdd/2026-08-14-model-v2-stage1-team-environment/task-6-report.md` and `final-review-fix-report.md`. Per the design's own "after this plan" instruction, Stage 2 was not started automatically — decide whether to proceed to Stage 2 (opportunity) as-is, investigate the league_mean gap first, or stop here, same as 1.15's own outcome. Before that: DST/K's replacement-level VOR fixed (streaming-aware, real historical data) and then DST/K excluded from the draft board entirely, per your own explicit workflow (see below) — both live-verified against the real 2026 board. See `docs/JOURNAL.md` for the full account. 1.15 remains a separate open item outside Phase 0/3 (paused mid-follow-up, see below); everything past Phase 0/1 is Phase 3 (SPEC §11.4's decomposed model v2 — Stage 1 above, Stages 2-4 not started — trade finder, empirical correlation estimation, season-long simulated rankings, route-data cost evaluation, public-readiness audit) — not started, not scoped in detail yet.
