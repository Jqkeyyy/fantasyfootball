...
**Last machine:** `Maybe` (Windows). Sleeper and all seven rankings sources (ESPN/FFToday fixed, FootballGuys/DraftSharks added this session) confirmed reachable live.
...
**Next task: undecided — the rankings-source repair + expansion you asked for this session is fully done.** ESPN/FFToday fixed, FootballGuys and DraftSharks added as two new ranks-only sources (7 total live now, up from 5). See `docs/JOURNAL.md` for the full account. 1.15 remains the project's only open item outside Phase 0/3 (paused mid-follow-up, see below); everything past Phase 0/1 is Phase 3 (SPEC §11.4's decomposed model v2, trade finder, empirical correlation estimation, season-long simulated rankings, route-data cost evaluation, public-readiness audit) — not started, not scoped in detail yet. FantasyData and Razzball (real per-stat tables, found but not built this session — see below) are the natural next candidates if more rankings coverage is wanted later.

This session: SPEC-ADDENDUM-03.md housekeeping (added to CLAUDE.md's reading order, tasks 0.15/0.16 added to TASKS.md, CLAUDE.md's "one league"/"six mandatory tests" lines corrected — now two leagues, eight tests per SPEC §16.3 including the two ADDENDUM-02 tests that were already mandated by name but never enumerated there); task 0.16 (static HTML export) built and verified live (real Chrome session via `claude-in-chrome`, zero network requests, real filter/search interaction against the real 854-player board); task 0.15 (mobile draft page) built and verified live on a real phone over LAN against a real replayed 2025 draft, including a new `ffapp draft live --replay/--stop` command and `draft/replay.py` module built to make that verification possible (ADDENDUM-03 §E's own replay-mode runbook step had never actually been implemented before this); **ESPN and FFToday rankings sources fixed** — ESPN needed a `view=kona_player_info` query param it silently didn't have before, FFToday needed a 1.5s delay between its six sequential position-page requests; **two new ranks-only sources added, FootballGuys (526 real players, one page) and DraftSharks (125 real players across 5 position pages, `Crawl-delay: 10` respected)** — `draft/board.py` generalised from one hardcoded rank-only source (FantasyPros) to a `_RANK_SOURCE_FETCHERS` dict, same shape as the existing point-source dict. Verified live: `ffapp draft board --no-offline` runs with zero skipped sources across all seven, board grew 854 → 1132 players over the session, every top-14-by-VOR player now shows `n_sources=7` (was capped at 3 at the start of this session). Full evidence for all of the above in `docs/JOURNAL.md`. `docs/summary.md` was not refreshed this session — still describes the state as of the prior close-out; it doesn't mention any of this session's work yet.
...
Nothing is mid-implementation. The rankings-source repair + expansion you asked for this session (fix ESPN/FFToday, then research/add up to 5 more sources) is fully done — both halves, see `docs/JOURNAL.md`. Other genuinely open threads, already named above, not repeated in full here:
