...
**Last machine:** `Maybe` (Windows). Sleeper and all five rankings sources (ESPN and FFToday included, both fixed this session) confirmed reachable live.
**Last commit:** see `git log` — this session's own work (SPEC-ADDENDUM-03 housekeeping + task 0.16) is committed and pushed by the end of this session; verify with `git status`/`git log origin/main` rather than trusting this line, same lesson the prior header's own wrong "pushed" claim already taught. Everything through `be21213` (the session before this one) was confirmed actually on `origin` before this session started.
...
**Next task: researching up to 5 more rankings sources, even partial-coverage ones (top 50-100 is fine).** ESPN and FFToday are fixed (see below) — the remaining half of your instruction is finding additional sources beyond the current five (FantasyPros, ESPN, FantasySharks, CBS Sports, FFToday). Not a numbered TASKS.md task — direct instruction from you this session. `docs/JOURNAL.md`'s 0.7 entry already documents three sites tried and rejected (NFL.com, Yahoo, Sharp Football Analysis — none had an accessible plain-HTTP data path); don't re-investigate those three.

After that: 1.15 remains the project's only open item outside Phase 0/3 (paused mid-follow-up, see below); everything past Phase 0/1 is Phase 3 (SPEC §11.4's decomposed model v2, trade finder, empirical correlation estimation, season-long simulated rankings, route-data cost evaluation, public-readiness audit) — not started, not scoped in detail yet.

This session: SPEC-ADDENDUM-03.md housekeeping (added to CLAUDE.md's reading order, tasks 0.15/0.16 added to TASKS.md, CLAUDE.md's "one league"/"six mandatory tests" lines corrected — now two leagues, eight tests per SPEC §16.3 including the two ADDENDUM-02 tests that were already mandated by name but never enumerated there); task 0.16 (static HTML export) built and verified live (real Chrome session via `claude-in-chrome`, zero network requests, real filter/search interaction against the real 854-player board); task 0.15 (mobile draft page) built and verified live on a real phone over LAN against a real replayed 2025 draft, including a new `ffapp draft live --replay/--stop` command and `draft/replay.py` module built to make that verification possible (ADDENDUM-03 §E's own replay-mode runbook step had never actually been implemented before this); **ESPN and FFToday rankings sources fixed** — ESPN needed a `view=kona_player_info` query param it silently didn't have before (confirmed live: without it, the endpoint returns 200 with `players` entirely absent), FFToday needed a 1.5s delay between its six sequential position-page requests (confirmed live: firing all six with no delay reliably 403s one of them; with the delay, all six succeed every time in repeated testing). Verified live: `ffapp draft board --no-offline` now runs with zero skipped sources, board grew 854 → 1123 players, every top-15-by-VOR player now shows `n_sources=5` (previously capped at 3). Full evidence for all of the above in `docs/JOURNAL.md`. `docs/summary.md` was not refreshed this session — still describes the state as of the prior close-out; it doesn't mention any of this session's work yet.
...
- [x] Ranking sources chosen — FantasyPros, ESPN, FantasySharks, CBS Sports, FFToday, **all five now live and contributing** (ESPN/FFToday fixed this session — see above). More sources may be added; see "Next task" above.
...
**Rankings-source repair + expansion — half done this session.** You asked, after noticing the board looked off with 2 of 5 sources dead: (1) fix ESPN (0 rows) and FFToday (403) if possible, (2) research up to 5 more ranking sources, partial top-50/100 coverage acceptable. **(1) is done** — see `docs/JOURNAL.md`'s new entry, both fixes verified live. **(2) not started yet** — resume by researching new candidate sites. `docs/JOURNAL.md`'s 0.7 entry already documents which sites were tried and rejected (NFL.com, Yahoo, Sharp Football Analysis — none had an accessible plain-HTTP data path) — don't re-investigate those three, look elsewhere.

Other genuinely open threads, already named above, not repeated in full here:
