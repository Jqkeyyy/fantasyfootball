...
**Last commit:** see `git log` — this session's own work (DST/K streaming-aware VOR + board exclusion, model v2 Stage 1's six-task plan, and its final whole-branch-review fix round, see below) is committed by the end of this session; verify with `git status`/`git log origin/main` rather than trusting this line, same lesson the prior header's own wrong "pushed" claim already taught. Everything through the prior session's own work was confirmed actually on `origin` before this session started.
...
**Next task: undecided on model v2 (see below); the runbook gap from the last session's own notes is now closed — `ffapp ingest rankings` exists and works.** New CLI command matching SPEC-ADDENDUM-03.md §E's "morning of" runbook step literally, verified live against real cached data (4/4 point sources, 3/3 rank sources, ADP all refresh cleanly). Full account in `docs/JOURNAL.md`'s newest entry.
