...
**Next task: undecided — everything you asked for this session is done.** Rankings-source repair + expansion (ESPN/FFToday fixed, FootballGuys/DraftSharks added, 7 sources live) and keepers restored to the board (visibly marked, all three real views) are both complete. See `docs/JOURNAL.md` for the full account. 1.15 remains the project's only open item outside Phase 0/3 (paused mid-follow-up, see below); everything past Phase 0/1 is Phase 3 (SPEC §11.4's decomposed model v2, trade finder, empirical correlation estimation, season-long simulated rankings, route-data cost evaluation, public-readiness audit) — not started, not scoped in detail yet.

This session: SPEC-ADDENDUM-03.md housekeeping (added to CLAUDE.md's reading order, tasks 0.15/0.16 added to TASKS.md, CLAUDE.md's "one league"/"six mandatory tests" lines corrected); task 0.16 (static HTML export) and task 0.15 (mobile draft page, with a new `ffapp draft live --replay/--stop` command) both built and verified live; **ESPN and FFToday rankings sources fixed**; **two new ranks-only sources added, FootballGuys (526 real players) and DraftSharks (125 real players, `Crawl-delay: 10` respected)** — `draft/board.py` generalised to a `_RANK_SOURCE_FETCHERS` dict; **keepers restored to the board, visibly marked** — prototyped safely against the dummy CSV first, then built into `draft/board.py` for real once you confirmed it (a new `is_keeper` column, no longer filtered out), with a distinct amber highlight + 🔒 marker in both the Streamlit board page and the static HTML export (the canonical `player` name itself stays clean everywhere — the marker is render-time only, so `draft.live`'s join-key matching during a real draft isn't affected). A real debugging detour here worth knowing about: the Streamlit page briefly *appeared* unstyled after this change even though the underlying Styler HTML was provably correct — turned out to be a stale long-running dev-server process that predated the code edit (Python module caching, not a real bug); restarting the server fixed it immediately. Board grew 854 → 1141 players over the session (rankings expansion + keepers restored). Full evidence for all of the above in `docs/JOURNAL.md`. `docs/summary.md` was not refreshed this session — still describes the state as of the prior close-out; it doesn't mention any of this session's work yet.
...
Nothing is mid-implementation. Everything asked for this session — rankings repair/expansion and keepers restored to the board — is fully done, verified live, and committed. Other genuinely open threads, already named above, not repeated in full here:
