...
Read `SPEC.md` §0 before doing anything else. Then read the addenda in order, each superseding or extending the sections it names: `SPEC-ADDENDUM-01.md` (multi-league support), `SPEC-ADDENDUM-02.md` (offline-first development), `SPEC-ADDENDUM-03.md` (phone access and draft-day operations), `SPEC-ADDENDUM-04.md` (rest-of-season weekly rankings — supersedes `SPEC.md` §11.3 and §14.2), `SPEC-ADDENDUM-05.md` (consensus as anchor, and the offseason model plan — supersedes `SPEC.md` §11.3's framing of the model's job; exactly one item in it is urgent, in-season prediction logging, everything else is explicitly offseason).
...
**Phase 0 and Phase 2 are complete. Phase 1's tasks 1.15 and 1.20 are closed** (not "done" — closed per `SPEC-ADDENDUM-04.md` §C's shipping rule: neither a from-scratch points model nor an anchored-residual model beat B2 robustly across three independent real tests, so `projection_source: consensus_b3` ships instead — see `docs/JOURNAL.md`'s 2026-08-16 entry). Tasks 1.21 (rest-of-season rankings pipeline) and 1.22 (cold-start source schedule) remain open. **`SPEC-ADDENDUM-05.md` (supersedes §11.3's framing of the model's job) reframes the model as a residual-correction layer on top of consensus, not a competitor to it**, and adds tasks 3.8–3.11 in `TASKS.md` (renumbered from the addendum's own literal "3.7–3.10" — that range collided with the pre-existing task 3.7, the DST/K streamer tab; the mapping is by section reference, §B/§C/§D/§E, not by number). **3.8 (in-season prediction logging, §B) is done, verified live** (`docs/JOURNAL.md`'s 2026-08-16 entry); 3.9 (B3-anchored restricted residual, §C), 3.10 (gated residual, §D), and 3.11 (learned source weighting, §E) are explicitly offseason/low-priority work, not to be started during the season. Every Phase 2 task (2.1–2.11) is done — several were built ahead of strict order once their own prerequisites turned out not to need Sleeper/unbuilt pipelines, each confirmed with the user first, see `docs/JOURNAL.md`. The decomposed model v2 line (Stages 1-3 built/evaluated, Stage 4 investigated and parked) stays Phase 3 per `SPEC-ADDENDUM-04.md` §F — "the right long-term architecture and the wrong thing to start four weeks before a season." Check `HANDOFF.md` §1 for the live, current picture; this line is a coarse pointer, not the source of truth.
