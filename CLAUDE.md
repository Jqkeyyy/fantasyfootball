...
**Phase 0 and Phase 2 are complete; Phase 1 has one open item, now with three new tasks queued behind it.** Phase 0 (draft board), including `SPEC-ADDENDUM-03.md` §F's two late-added draft-blocking tasks (0.15 mobile draft page, 0.16 static HTML export), is fully shipped. Phase 1's open item is 1.15 (conditional points model v1 — built, doesn't beat the B2 baseline on Spearman-within-position-week). **`SPEC-ADDENDUM-04.md` (supersedes §11.3/§14.2) reopens 1.15's own diagnosis** — the prior hyperparameter-search conclusion that the gap is "architectural" is disputed there as premature (a tuning search can't distinguish an architecture problem from a feature/target/measurement problem); do not treat 1.15 as closed-architectural until that addendum's own §A diagnostics (five cheap tests) have actually been run. New tasks 1.20 (anchored residual model, if §A doesn't clear the bar), 1.21 (rest-of-season rankings pipeline), 1.22 (cold-start source schedule) queue behind 1.15 per that addendum's own sequencing — not started as of this writing, see `HANDOFF.md` §1 for the live status. Every Phase 2 task (2.1–2.11) is done — several were built ahead of strict order once their own prerequisites turned out not to need Sleeper/unbuilt pipelines, each confirmed with the user first, see `docs/JOURNAL.md`. Everything past this point is Phase 3 (SPEC-ADDENDUM-04.md §F: the decomposed model v2 line stays Phase 3 explicitly, "the right long-term architecture and the wrong thing to start four weeks before a season"). Check `HANDOFF.md` §1 for the live, current picture; this line is a coarse pointer, not the source of truth.
